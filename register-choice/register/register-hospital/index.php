...
<?php
 ...
 require 'connect.ini.php';
  ...
  if(isset($_POST['submit']))
  {
    ...
    $nam=$_POST['hospital-name'];
    $em=$_POST['email'];
    $mob=($_POST['mobile']);
    $citi=$_POST['city'];
    $add=$_POST['address'];
    
     if(isset($_POST['hospital-name']) && isset($_POST['password']) && isset($_POST['user-name']) && isset($_POST['email']) && isset($_POST['mobile']) && isset($_POST['address']) && isset($_POST['city']) && isset($_POST['term']) && !empty($_POST['hospital-name']) && !empty($_POST['user-name']) && !empty($_POST['email']) && !empty($_POST['mobile']) &&  !empty($_POST['city']) && !empty($_POST['address']))
  {
  ...
  //  $query='INSERT INTO hospital(name,address,city,mobile,email,username) VALUES ("'.$nam.'","'.$add.'","'.$citi.'",'.$mob.'","'.$em.'","'.$uname.');';
     $query= 'INSERT INTO `hospital`(`name`, `address`, `city`, `mobile`, `email`, `username`, `password`) VALUES ("'.$nam.'","'.$add.'","'.$citi.'","'.$mob.'","'.$em.'","'.$uname.'","'.$pass.'");';
    
   // $query='select * from hospital';
    if(mysqli_query($con,$query))
    {
      echo "You are registered! Thank You!";
        $query= "INSERT INTO `hblood`(`username`, `bloodG`, `units`) VALUES ('$uname','A+',0)";
     mysqli_query($con,$query);
      $query= "INSERT INTO `hblood`(`username`, `bloodG`, `units`) VALUES ('$uname','A-',0)";
     mysqli_query($con,$query);
      $query= "INSERT INTO `hblood`(`username`, `bloodG`, `units`) VALUES ('$uname','B+',0)";
     mysqli_query($con,$query);
      $query= "INSERT INTO `hblood`(`username`, `bloodG`, `units`) VALUES ('$uname','B-',0)";
     mysqli_query($con,$query);
      $query="INSERT INTO `hblood`(`username`, `bloodG`, `units`) VALUES ('$uname','AB+',0)";
     mysqli_query($con,$query);
      $query= "INSERT INTO `hblood`(`username`, `bloodG`, `units`) VALUES ('$uname','AB-',0)";
     mysqli_query($con,$query);
      $query= "INSERT INTO `hblood`(`username`, `bloodG`, `units`) VALUES ('$uname','O+',0)";
     mysqli_query($con,$query);
      $query= "INSERT INTO `hblood`(`username`, `bloodG`, `units`) VALUES ('$uname','O-',0)";
     mysqli_query($con,$query);
     $_SESSION['name']=$uname;
    header('Location: http://localhost/Blood/login-choice/login/log-hospital/');
    }
    else
    {
...
<h1 style="color:yellow;"><center><strong>REGISTER AS A HOSPITAL</strong></center></h1>
...
?>
      <div class="input-group input-group-icon">
        <input type="text" name='hospital-name' value='<?php echo $nam; ?>' placeholder="Name of the Hospital"/>
